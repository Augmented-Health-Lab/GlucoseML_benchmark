...
class Encoder_PCA(nn.Module):
    def __init__(self, input_dim, word_embedding, hidden_dim=768, num_heads=12, num_encoder_layers=1):
        ...
        self.word_embedding = word_embedding.T  # expect shape (T, hidden_dim)

    def forward(self, x):
        # x: (B, M, input_dim)
        B = x.shape[0]

        # Make word embeddings batch-aligned: (B, T, hidden_dim)
        if self.word_embedding.ndim == 2:
            we = self.word_embedding.unsqueeze(0).repeat(B, 1, 1)
        elif self.word_embedding.shape[0] != B:
            we = self.word_embedding[0:1].repeat(B, 1, 1)
        else:
            we = self.word_embedding

        x = self.linear(x)                 # (B, M, hidden_dim)
        x = self.transformer_encoder(x)    # (B, M, hidden_dim)

        x_time = x

        # Cross-attention: query=time tokens, key/value=text tokens
        x, _ = self.cross_attention(x, we, we)  # (B, M, hidden_dim)
