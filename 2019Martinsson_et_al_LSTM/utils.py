...
def dataframe_to_supervised(df, nb_past_steps, nb_steps_in_future, stride=1):
    xs = []
    ys = []
    for column in df.columns:
        sequence = df[column].as_matrix()
        x, y = sequence_to_supervised(sequence, nb_past_steps,
                nb_steps_in_future, stride)
...
def sequence_to_supervised(data, nb_past_steps, nb_steps_in_future, stride=1):
    """
    Sliding window with proper target alignment and stride.
    """

    total = len(data) - nb_past_steps - nb_steps_in_future + 1
    if total <= 0:
        return np.array([]), np.array([])

    x = np.array([
        data[i : i + nb_past_steps]
        for i in range(0, total, stride)
    ])

    y = np.array([
        data[i + nb_past_steps + nb_steps_in_future - 1]
        for i in range(0, total, stride)
    ])
...
def sequence_to_supervised_all(data, nb_past_steps, nb_future_steps, stride=1):
