...
def generate_yaml_configs(
    ...
    new_yaml_dir = os.path.join(
        base_yaml_root,
        f"full_shot_open_dataset_eval_{horizon}_{dataset}"
    )
    ...
    for csv_file in csv_files:
        ...
        yaml_content = {
            ...
            "train": {
                ...
                "artifacts_path": f"../../artifacts/martinsson_kdd_experiment_{nb_past_steps}sh_full_{horizon}/",
                "batch_size": 1024,
                "epochs": 10000,
                "param_seed": seed,
                "patience": 50,
...
def generate_fullshot_train_yaml(
    output_root,
    training_data_path,
    nb_past_steps=144,
    param_nb_future_steps=[6],
    ...
    yaml_filename = f"full_shot_open_dataset_train_{horizon}.yaml"
...
def evaluate(model, x_test, y_test, cfg):
    ...
    results_csv_path = os.path.join(dataset_output_dir, f"{patient_id}_predictions.csv")
    results_df.to_csv(results_csv_path, index=False)

    print(f"Saved metrics and predictions to: {dataset_output_dir}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Run LSTM full-shot evaluation on controlled datasets from existing YAML folders."
    )
    parser.add_argument(
        "--horizon", required=True,
        help="Comma-separated forecast horizons, e.g. '3' or '3,6,12,18'.",
    )
    parser.add_argument(
        "--dataset", default="ohio,diatrend,t1dexi",
        help="Comma-separated dataset tags, e.g. 'ohio' or 'ohio,diatrend,t1dexi'.",
    )
    parser.add_argument(
        "--base_root",
    ...
    for horizon in horizons:
        for dataset_tag in datasets:
            eval_dir = os.path.join(
                args.base_root,
                f"full_shot_open_dataset_eval_{horizon}_{dataset_tag}",
            )
            print("\n" + "#" * 80)
            print(f"# horizon={horizon}  dataset={dataset_tag}")
            print(f"# eval_dir={eval_dir}")
            print("#" * 80)

            if not os.path.isdir(eval_dir):
                logging.error(f"Eval YAML directory not found, skipping: {eval_dir}")
                continue
