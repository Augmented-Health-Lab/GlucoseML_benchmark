...
def main():
    parser = argparse.ArgumentParser(
        description="Run LSTM few-shot evaluation on controlled datasets from existing YAML folders."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--base_root",
        default="/content/drive/Shareddrives/Baiying/2019Martinsson_et_al_LSTM/glucofm_fewshot_open_144",
        help="Root containing the few_shot_open_dataset_eval_<horizon>_<dataset> folders.",
    )
    ...
    for horizon in horizons:
        for dataset_tag in datasets:
            eval_dir = os.path.join(
                args.base_root,
                f"few_shot_open_dataset_eval_{horizon}_{dataset_tag}",
            )
            print("\n" + "#" * 80)
