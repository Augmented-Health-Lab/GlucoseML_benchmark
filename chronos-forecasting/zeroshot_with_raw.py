...
def rolling_window_forecast(sequences, pipeline, context_length, prediction_length=18, step_size=1, verbose=False):
    ...
    Returns
    -------
    all_predictions, all_ground_truth, all_timestamps, all_sequence_ids
    """
    all_predictions = []
    all_ground_truth = []
    all_timestamps = []
...
def collect_raw_pred_records(
    ...
    """Collect per-window (Prediction, GroundTruth) pairs for each horizon."""
    records = []
    for horizon_name, h in horizon_steps.items():
        for win_idx, (pred, gt) in enumerate(zip(all_predictions, all_ground_truth)):
            if len(pred) < h or len(gt) < h:
                continue
            records.append({
                ...
                "Window_Idx": win_idx,
                "Prediction": float(pred[h - 1]),
                "GroundTruth": float(gt[h - 1]),
            })
    return records
...
def evaluate_single_patient(df, patient_name, pipeline, context_lengths,
    ...
    try:
        ...
        patient_results = {}
        raw_records = [] if collect_raw else None
        ...
        if collect_raw:
            return patient_results, raw_records
...
def main(args):
    ...
    for dataset_name in dataset_names:
        ...
        for i, subject in enumerate(subset, 1):
            ...
            patient_results, raw_records = evaluate_single_patient(
                df=df,
        ...
        out_dir = Path(f"./{args.results_dir}/{dataset_name}/step_{args.step_size}")
        ...
        if all_raw_records:
            ...
            print(f"Saved raw predictions to {raw_path}  "
                  ...
                  f"horizons={raw_df['Horizon'].unique().tolist()})")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--results_dir", type=str, default="results_with_raw",
                        help="Top-level output directory.")
    parser.add_argument("--raw_csv", type=str,
                        default="zeroshot_raw_predictions.csv",
