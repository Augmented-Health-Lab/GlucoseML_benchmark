...
# =====================================================
# BUILD FEW-SHOT TRAIN INPUTS (HF)
# =====================================================
...
# =====================================================
# EVALUATION (HF) — with raw per-window outputs
# =====================================================
...
def evaluate_hf_split(
        ...
        for subject in ds[split]:
            ...
            for name, h in horizon_steps.items():
                ...
                if save_predictions_path is not None:
                    for win_idx, (pred_val, gt_val) in enumerate(
                        zip(preds_all[:, h - 1], gts_all[:, h - 1])
                    ):
    ...
    if save_predictions_path is not None:
        raw_pred_df = pd.DataFrame(raw_pred_records)
        raw_pred_df.to_csv(save_predictions_path, index=False)
        print(f"\nRaw predictions saved to: {save_predictions_path}")
        print(f"  Shape: {raw_pred_df.shape}  |  "
              f"Horizons: {raw_pred_df['Horizon'].unique().tolist()}")
...
def main(args):
    ...
    summary_df, detailed_df = evaluate_hf_split(
        ...
        gap_threshold_hours=args.gap_hours,
        save_predictions_path=results_dir / args.raw_csv,
    )

    summary_df.to_csv(results_dir / args.summary_csv, index=False)
    detailed_df.to_csv(results_dir / args.detailed_csv, index=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_model", type=str,
                        default="./chronos2_lora_fewshot")

    parser.add_argument("--results_dir", type=str,
                        default="results_with_raw",
                        help="Output directory for summary / detailed / raw CSVs.")

    parser.add_argument("--summary_csv", type=str,
                        default="fewshot_summary.csv")

    parser.add_argument("--detailed_csv", type=str,
                        default="fewshot_detailed.csv")

    parser.add_argument("--raw_csv", type=str,
                        default="fewshot_raw_predictions.csv",
                        help="Per-window (Prediction, GroundTruth) CSV "
                             "for Clarke / Surveillance Error Grid analysis.")
