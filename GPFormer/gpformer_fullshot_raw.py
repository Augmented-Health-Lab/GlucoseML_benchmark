...
DEFAULT_RESULTS_ROOT = Path(__file__).resolve().parent / "multi_horizon_results_gpformer_fullshot_raw"
DEFAULT_LOG_STEM = "predict_glucose_multiwindow_gpformer_fullshot_raw"
...
class GPFormerWindowDataset(
    ...
    def __getitem__(
        ...
        return (
            torch_tensor_from_array(past_scaled, dtype=torch.float32).unsqueeze(-1),
            torch_tensor_from_array(past_mark.astype("float32"), dtype=torch.float32),
            torch_tensor_from_array(dec_inp, dtype=torch.float32).unsqueeze(-1),
            torch_tensor_from_array(dec_marks.astype("float32"), dtype=torch.float32),
            torch_tensor_from_array(future_scaled, dtype=torch.float32).unsqueeze(-1),
        )
    ...
    *,
    ...
    parser = argparse.ArgumentParser(
        description=(
            ...
            "Also writes per-window raw predictions and ground truths."
        )
    )
    ...
    *,
    ...
    for cfg in configs:
        ...
        for dataset_name, info in dataset_state.items():
            ...
            prediction_records = info["prediction_records"]
            ...
            for participant_id, subject_payload in subject_iter:
                ...
                if pred_rows:
                    for row in pred_rows:
                        ...
                        row.setdefault("model_name", args.model_name)
        ...
        for dataset_name, info in dataset_state.items():
            ...
            predictions_path: Path = info["predictions_path"]
            prediction_records: List[Dict[str, object]] = info["prediction_records"]
            if prediction_records:
                ...
                LOGGER.info(
                    "Saved predictions to %s (+%d rows)", predictions_path, len(pred_df)
                )
