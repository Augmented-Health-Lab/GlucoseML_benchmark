...
RESULTS_ROOT = Path(__file__).resolve().parent / "multi_horizon_results_timesfm_zeroshot"
...
def setup_logging() -> Path:
    ...
    log_path = RESULTS_ROOT / f"predict_glucose_multiwindow_timesfm_zeroshot_{timestamp}.log"
    logging.basicConfig(
        level=logging.INFO,
...
def evaluate_subject(
    ...
    batch_size: int,
    metric_mode: str,
) -> Tuple[float, float, int, int, List[Dict]]:
    ...
    # accumulates one dict per evaluated prediction
    raw_preds: List[Dict] = []
        ...
        for i, (pred_vec, target_vals) in enumerate(zip(preds, batch_targets)):
            ...
            for step_idx in range(valid_len):
                raw_preds.append({
                    ...
                    "prediction": float(pred_vec[step_idx]),
    ...
    flush_batch()
    return total_sse, total_ae, total_points, total_windows, raw_preds
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Zero-shot: evaluate TimesFM on 5-minute CGM data for multiple context windows and horizons."
    )
    parser.add_argument(
        "--task",
        choices=["eval", "latest"],
...
def main() -> None:
    ...
    for split_name in args.splits:
        ...
        for dataset_dir in dataset_dirs:
            ...
            raw_output_path = RESULTS_ROOT / f"{dataset_dir.name}_{split_name}_raw_predictions.csv"

            existing_df = pd.DataFrame()
            completed_keys: Set[Tuple[str, int, int]] = set()
            completed_raw_keys: Set[Tuple[str, int, int]] = set()
            if not args.overwrite:
                ...
                # Load existing raw predictions to know which keys are already saved
                if raw_output_path.exists():
                    try:
                        existing_raw_df = pd.read_csv(raw_output_path)
                        raw_key_cols = {"participant_id", "context_hours", "horizon_minutes"}
                        if raw_key_cols.issubset(existing_raw_df.columns):
                            completed_raw_keys = set(
                                (
                                    str(row["participant_id"]),
                                    int(row["context_hours"]),
                                    int(row["horizon_minutes"]),
                                )
                                for _, row in existing_raw_df.iterrows()
                            )
                            LOGGER.info(
                                "%s (%s split): found %d existing raw prediction keys.",
                                dataset_dir.name, split_name, len(completed_raw_keys),
                            )
                    except Exception as exc:
                        LOGGER.warning("Failed to load existing raw predictions from %s (%s)", raw_output_path, exc)

            subject_groups = collect_subject_groups(dataset_dir)
            records: List[Dict] = []
            raw_pred_records: List[Dict] = []
            processed_counter = 0
            ...
            for participant_id, csv_paths in tqdm(
                ...
                for cfg in configs:
                    ...
                    metrics_done = key in completed_keys
                    raw_done = key in completed_raw_keys

                    # Skip only if BOTH metrics and raw predictions are already complete
                    ...
                    if args.task == "latest":
                        ...
                        try:
                            with torch.inference_mode():
                                preds, _ = model.forecast(horizon=cfg.horizon_steps, inputs=[history])
                        ...
                        sse, sae, points, windows, raw_preds = evaluate_subject(
                            model,
                            participant_id,
                            df,
                            cfg,
                        ...
                        # Only write new metric records if not already computed
                        if not metrics_done:
                        ...
                        # Always collect raw predictions if not already saved
                        if not raw_done:
                            for rp in raw_preds:
                ...
                if processed_counter % CHECKPOINT_EVERY == 0 and records:
                    ...
                    # also checkpoint raw predictions
                    if raw_pred_records:
                        raw_tmp_path = raw_output_path.with_suffix(".checkpoint.csv")
                        pd.DataFrame(raw_pred_records).to_csv(raw_tmp_path, index=False)
                        LOGGER.info("Raw predictions checkpoint: %s (%d rows)", raw_tmp_path, len(raw_pred_records))
            ...
            # --- save final raw predictions ---
            if raw_pred_records:
                new_raw_df = pd.DataFrame(raw_pred_records)
                if not args.overwrite and raw_output_path.exists():
                    try:
                        existing_raw_df = pd.read_csv(raw_output_path)
                        new_raw_df = pd.concat([existing_raw_df, new_raw_df], ignore_index=True)
                    except Exception as exc:
                        LOGGER.warning("Could not load existing raw predictions for merge (%s)", exc)
                new_raw_df = new_raw_df.sort_values(
                    ["participant_id", "context_hours", "horizon_minutes", "window_start"]
                ).reset_index(drop=True)
                new_raw_df.to_csv(raw_output_path, index=False)
                LOGGER.info("Saved raw predictions to %s (%d rows)", raw_output_path, len(new_raw_df))
