...
def load_timesfm_model(
    ...
    # TimesFM compiles a fixed-shape decode graph. Its max_context/max_horizon must
    # align to internal patch sizes; otherwise the library will round them up and
    # log a message. We round explicitly here to make the behavior predictable.
...
def main() -> None:
    ...
    model = load_timesfm_model(
        args.model_name, max_context_steps=max_context_steps, max_horizon_steps=max_horizon_steps, batch_size=args.batch_size
    )
    ...
    for split_name in args.splits:
        ...
        if args.data_source == "hf":
            ...
            dataset_iter = ((name, subjects) for name, subjects in dataset_items)
        else:
